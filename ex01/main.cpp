...
/*   Updated: 2026/01/16 17:45:55 by toruinoue        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 値渡しで受け取る関数
void print_int_by_value(int value)
{
	std::cout << value << " ";
}
...
void multiply_float(float &value)
{
	value *= 2.0f;
}
	...
	while (true) {
		...
		switch (choice[0]) {
			...
			case '0':
				...
				return 0;
			default:
				std::cout << RED_COLOR << "無効な選択です。0-6の数字を入力してください。" << RESET_COLOR << std::endl;
				break;
		}
		std::cout << std::endl;
	}
